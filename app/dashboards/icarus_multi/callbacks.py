...
def _load_multi_data(report_date, cohort, metrics, plan_values, plan_more_values, theme, active_inactive):
    ...
    try:
        ...
        if df_regular is not None and not df_regular.empty:
            pivot_content.append(grid_section(
                "Plans Overview (Regular)",
                _build_multi_grid(df_regular, theme),
                "multi-pivot-regular",
                colors
            ))
        ...
        if df_crystal is not None and not df_crystal.empty:
            ...
            pivot_content.append(grid_section(
                "Plans Overview (Crystal Ball)",
                _build_multi_grid(df_crystal, theme),
                "multi-pivot-crystal",
                colors
            ))
